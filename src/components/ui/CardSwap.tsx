...
const makeSlot = (i: number, distX: number, distY: number, total: number, reflect = false) => ({
  x: reflect ? -i * distX : i * distX,
  y: -i * distY,
  z: reflect ? i * distX * 1.5 : -i * distX * 1.5,
  zIndex: total - i,
});
...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  useEffect(() => {
    ...
    refs.forEach((r, i) => {
      if (r.current) placeNow(r.current, makeSlot(i, cardDistance, verticalDistance, total, reflect), skewAmount);
    });
    ...
    const swap = () => {
      ...
      rest.forEach((idx, i) => {
        ...
        if (!el) return;
        const slot = makeSlot(i, cardDistance, verticalDistance, refs.length, reflect);
        tl.set(el, { zIndex: slot.zIndex }, 'promote');
        tl.to(el, { x: slot.x, y: slot.y, z: slot.z, duration: config.durMove, ease: config.ease }, `promote+=${i * 0.15}`);
      });

      const backSlot = makeSlot(refs.length - 1, cardDistance, verticalDistance, refs.length, reflect);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cardDistance, verticalDistance, delay, pauseOnHover, skewAmount, easing, reflect]);
